            ...
            <div class="row">
              <div class="col-md-6 mb-2">
                ...
                <div class="form-group">
                  ...
                  <input type="text" readonly class="readonly form-control <?= ($error = validation_show_error('mat_sap_code')) ? 'border-danger' : ''; ?>" id="mat_sap_code" name="mat_sap_code" placeholder="Mat Sap Code" value="<?= old('mat_sap_code'); ?>">
                  <?= ($error) ? '<span class="error text-danger mb-2">' . $error . '</span>' : ''; ?>
                </div>

                <div class="form-group">
                  <label for="mat_desc">Mat Desc</label>
                  <input type="text" readonly class="readonly form-control <?= ($error = validation_show_error('mat_desc')) ? 'border-danger' : ''; ?>" id="mat_desc" name="mat_desc" placeholder="Mat Desc" value="<?= old('mat_desc'); ?>">
                  <?= ($error) ? '<span class="error text-danger mb-2">' . $error . '</span>' : ''; ?>
                </div>
              </div>
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input type="number" class="form-control <?= ($error = validation_show_error('target_shift')) ? 'border-danger' : ''; ?>" id="target_shift" name="target_shift" placeholder="Target Shift" value="<?= old('target_shift'); ?>">
                  <?= ($error) ? '<span class="error text-danger mb-2">' . $error . '</span>' : ''; ?>
                </div>

                <div class="form-group">
                  <label for="target_hour">Target Hour</label>
                  <input type="number" readonly class="form-control <?= ($error = validation_show_error('target_hour')) ? 'border-danger' : ''; ?>" id="target_hour" name="target_hour" placeholder="Target Hour" value="<?= old('target_hour'); ?>">
                  <?= ($error) ? '<span class="error text-danger mb-2">' . $error . '</span>' : ''; ?>
                </div>

                <div class="form-group">
                  <label for="target_minute">Target Minute</label>
                  <input type="number" readonly class="form-control <?= ($error = validation_show_error('target_minute')) ? 'border-danger' : ''; ?>" id="target_minute" name="target_minute" placeholder="Target Minute" value="<?= old('target_minute'); ?>">
                  <?= ($error) ? '<span class="error text-danger mb-2">' . $error . '</span>' : ''; ?>
                </div>
