        ...
        <form action="<?= base_url($_page->link . '/' . $data['id']); ?>" method="post">
          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <input type="text" class="form-control <?= (validation_show_error('title')) ? 'border-danger text-danger' : ''; ?>" id="title" name="title" placeholder="Enter Name" value="<?= $data['title']; ?>">
                      ...
                      <?php if (!empty($permissions = model('App\Models\PermissionModel')->findAll())) : ?>
                        <?php foreach ($permissions as $row) : ?>
                          <tr>
                            ...
                            <?php
                            $isChecked = in_array($row['id'], $role_permissions) ? 'checked' : '';
                            ?>
                            <td width="50" class="text-center"><input type="checkbox" class="check-select-p" name="permission[]" value="<?= $row['id']; ?>" <?= $isChecked; ?>></td>
                          </tr>
                        <?php endforeach ?>
                      <?php else : ?>
